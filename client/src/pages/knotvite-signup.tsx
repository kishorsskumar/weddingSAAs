...
import { Heart, Mail, Lock, User, ArrowRight, Check, Gift, Star, Crown, Loader2, Phone as PhoneIcon } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#e0f4f8] via-white to-[#f0fafb]">
            ...
            <div className="flex items-center gap-2 text-sm">
              {mode === "signup" ? (
                <>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => { setMode("signin"); setError(""); }} data-testid="nav-signin">Sign In</Button>
                </>
              ) : (
                <>
                  <span className="text-gray-500 hidden sm:inline">New to KnotVite?</span>
                  <Button variant="ghost" size="sm" onClick={() => { setMode("signup"); setError(""); }} data-testid="nav-signup">Sign Up</Button>
                </>
              )}
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 sm:py-12">
        <div className={`grid ${mode === "signup" ? "md:grid-cols-5" : "md:grid-cols-1 max-w-md mx-auto"} gap-6 lg:gap-10`}>
          {mode === "signup" && (
          ...
          <div className={mode === "signup" ? "md:col-span-3" : ""}>
            <Card className="shadow-xl border-0" data-testid={mode === "signup" ? "signup-form-card" : "signin-form-card"}>
              <CardContent className="p-6 sm:p-8">
                {mode === "signin" && (
                  <div className="text-center mb-6">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3" style={{ backgroundColor: TEAL }}>
                      <Heart className="h-6 w-6 text-white fill-white" />
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900">Welcome Back</h1>
                    <p className="text-gray-500 text-sm mt-1">Sign in to your KnotVite account</p>
                  </div>
                )}
                ...
                <form onSubmit={mode === "signup" ? handleSignup : handleSignin} className="space-y-5">
                  ...
                  <Button
                    ...
                    disabled={submitting}
                    data-testid={mode === "signup" ? "submit-signup" : "submit-signin"}
                  >
                    {submitting ? (
                      <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> {mode === "signup" ? "Creating Account..." : "Signing In..."}</>
                    ) : mode === "signup" ? (
                      planInfo.trial ? (
                  ...
                  <div className="text-center text-sm text-gray-500">
                    {mode === "signup" ? (
                      <p>
                        ...
                        <button type="button" onClick={() => { setMode("signin"); setError(""); }} className="font-medium hover:underline" style={{ color: TEAL }} data-testid="switch-to-signin">
                          Sign In
                        </button>
                      </p>
                    ) : (
                      <p>
                        ...
                        <button type="button" onClick={() => { setMode("signup"); setError(""); }} className="font-medium hover:underline" style={{ color: TEAL }} data-testid="switch-to-signup">
                          Sign Up Free
                        </button>
                      </p>
                    )}
