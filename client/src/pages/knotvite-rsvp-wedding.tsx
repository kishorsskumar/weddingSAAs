...
import { Calendar, MapPin, Clock, Heart, Loader2, Sparkles, Search, UserPlus, User } from "lucide-react";
...
function extractNames(title: string): { name1: string; name2: string; taglineFromTitle?: string } {
  ...
  if (cleanTitle.split(' ').length <= 4 && cleanTitle.split(' ').length >= 2) {
...
export default function KnotviteRsvpWedding() {
  const [, params] = useRoute("/knotvite/rsvp/:slug");
  ...
  const primaryColor = lp.primaryColor || '#0d9488';
  ...
  const nameData = useMemo(() => {
    ...
    if (eventInfo.groomName || eventInfo.brideName) {
      return { name1: eventInfo.groomName || '', name2: eventInfo.brideName || '', taglineFromTitle: undefined as string | undefined };
    }
    return extractNames(eventInfo.invitationTitle || eventInfo.title);
  ...
  const eventType = (eventInfo?.eventType || 'wedding').toLowerCase();
  ...
  const handleSearchChange = useCallback((value: string) => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      setSearching(true);
      try {
        const res = await fetch(`/api/knotvite/public/rsvp/${slug}/search?name=${encodeURIComponent(value.trim())}`);
        const data = await res.json();
  ...
  const handleSelectGuest = (guestId: string) => {
    setLocation(`/knotvite/rsvp/${slug}/respond?guest=${guestId}`);
  };
  ...
  const handleSelfRegister = async () => {
    ...
    try {
      ...
      setLocation(`/knotvite/rsvp/${slug}/respond?guest=${guest.id}`);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] overflow-x-hidden" data-testid="knotvite-rsvp-wedding-page">
      <style>{`
      ...
      <section className="relative h-screen flex flex-col items-end justify-end overflow-hidden" data-testid="hero-section">
        ...
        <div className="relative z-10 text-center text-white px-4 sm:px-6 pb-6 sm:pb-12 w-full max-w-2xl mx-auto flex flex-col items-center">
          ...
          <div className="animate-fade-up-d1 mb-4">
            <h1 className="font-serif-display text-4xl sm:text-5xl md:text-6xl font-medium leading-[0.95] mb-2 shimmer-text" data-testid="text-groom-name">
              {nameData.name1}
            </h1>
            {nameData.name2 && (
              <>
                ...
                <h1 className="font-serif-display text-4xl sm:text-5xl md:text-6xl font-medium leading-[0.95] shimmer-text" data-testid="text-bride-name">
                  {nameData.name2}
                </h1>
              </>
            )}
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              {showCeremonies && eventInfo.ceremonies && eventInfo.ceremonies.length > 0 && (
                <div className="mt-10 pt-8 border-t border-gray-100">
                  <p className="font-sans-clean text-[10px] tracking-[0.5em] uppercase mb-5" style={{ color: accentColor }}>
                    Ceremonies
                  </p>
                  <div className="flex flex-wrap justify-center gap-3">
                    {eventInfo.ceremonies.map((fn, i) => (
                      <span
                        key={i}
                        className="font-sans-clean text-xs px-3 py-2 sm:px-5 sm:py-2.5 rounded-full border-2 transition-colors duration-300 hover:text-white cursor-default"
                        style={{
              ...
              <div className="flex items-center justify-center gap-3 sm:gap-6">
                {[
                  ...
                  <div key={i} className="text-center" data-testid={`countdown-${item.label.toLowerCase()}`}>
                    <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-2xl glass-card shadow-md border border-white/60 flex items-center justify-center mb-2">
              ...
              <div className="mt-4 bg-white/90 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100 animate-in fade-in slide-in-from-bottom-3 duration-300" data-testid="self-register-form">
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    style={{ backgroundColor: primaryColor, height: '48px', borderRadius: '12px' }}
                    onClick={handleSelfRegister}
                    disabled={registering || selfRegName.trim().length < 2 || selfRegPhone.trim().length < 5}
                    data-testid="button-register-submit"
                  >
                    {registering ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Continue to RSVP'}
                  </Button>
                </div>
              </div>
            )}
      ...
      <footer className="relative z-10 py-12 text-center bg-[#faf8f5]" data-testid="footer">
        <div className="flex items-center justify-center gap-3 mb-3">
          <div className="h-px w-12" style={{ background: `linear-gradient(90deg, transparent, ${accentColor}30)` }} />
          <Heart className="w-3 h-3" style={{ color: `${accentColor}60` }} />
          <div className="h-px w-12" style={{ background: `linear-gradient(270deg, transparent, ${accentColor}30)` }} />
        </div>
        <p className="font-serif-elegant text-sm text-gray-400 italic">{footerMessage}</p>
        <p className="font-sans-clean text-[9px] tracking-[0.3em] uppercase text-gray-300 mt-2">Powered by KnotVite</p>
      </footer>
    </div>
  );
}
