...
// Notifications - User notifications for events, deadlines, and system alerts
export const notifications = pgTable("notifications", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  type: text("type").notNull(), // 'event_reminder', 'invoice_due', 'estimate_due', 'leave_request', 'leave_approved', 'leave_rejected', 'production_deadline', 'system'
  title: text("title").notNull(),
  message: text("message").notNull(),
  priority: text("priority").notNull().default('normal'), // 'low', 'normal', 'high', 'urgent'
  isRead: boolean("is_read").notNull().default(false),
  isDismissed: boolean("is_dismissed").notNull().default(false),
  relatedEntityType: text("related_entity_type"), // 'event', 'invoice', 'estimate', 'leave_request', 'production_task'
  relatedEntityId: varchar("related_entity_id"),
  actionUrl: text("action_url"), // Deep link to related page
...
// Notification Preferences - Per-user settings for notification types and timing
export const notificationPreferences = pgTable("notification_preferences", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: 'cascade' }),
  // Event reminders
  eventRemindersEnabled: boolean("event_reminders_enabled").notNull().default(true),
  eventReminderDays: integer("event_reminder_days").notNull().default(3), // Days before event
  // Invoice/Estimate due date reminders
  invoiceDueRemindersEnabled: boolean("invoice_due_reminders_enabled").notNull().default(true),
  invoiceReminderDays: integer("invoice_reminder_days").notNull().default(3),
  estimateDueRemindersEnabled: boolean("estimate_due_reminders_enabled").notNull().default(true),
  estimateReminderDays: integer("estimate_reminder_days").notNull().default(5),
  // Leave request notifications
  leaveRequestNotificationsEnabled: boolean("leave_request_notifications_enabled").notNull().default(true),
  // Production planning notifications
  productionDeadlineRemindersEnabled: boolean("production_deadline_reminders_enabled").notNull().default(true),
  productionReminderDays: integer("production_reminder_days").notNull().default(2),
  // General settings
